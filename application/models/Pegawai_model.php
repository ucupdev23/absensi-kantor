...
class Pegawai_model extends CI_Model
{
    ...
    public function delete($id)
    {
        $emp = $this->db->get_where($this->table, ['id' => $id])->row();
        if ($emp) {
            // soft-delete: ubah status jadi nonaktif, bukan hapus permanen
            $this->db->where('id', $id)->update($this->table, ['status' => 'nonaktif']);
            $this->db->where('id', $emp->user_id)->update('users', ['status' => 'nonaktif']);
        }
    }
    ...
    /**
     * Ambil daftar pegawai aktif (untuk dropdown penugasan, dll)
     * Dipindah dari Employee_model
     */
